...
from .utils import expand_output_layer, compute_conv_out_shape
...
class CNN(nn.Module):
    ...
    def __init__(self, in_channels, device, H, W, n_conv_layers=3, feed_layers=[256, 128], 
        ...
        out_chs = [self.in_channels] + \
            [ 16*(i+1) for i in range(self.n_conv_layers) ]
        ...
        for i in range(self.n_conv_layers):
            W, H = compute_conv_out_shape(W, H, 0, 1, ks[i], 1)
            self.layers = nn.ModuleDict([
                [f'conv{i}', nn.Conv2d(out_chs[i], out_chs[i+1], kernel_size=ks[i], stride=2)],
                [f'relu{i}', nn.ReLU()],
        ...
        for i, el in enumerate(feed_layers):
            ...
            self.layers.update([
                [f'l{i+1}', nn.Linear(input_size, el, bias=True)],
                [f'relu_l{i+1}', nn.ReLU()],
            ])
    ...
    def forward(self, x):
        ...
        for i in range(len(self.feed_layers)):
            h = self.layers[f'l{i+1}'](h)
            hs.append(h)
            h = self.layers[f'relu_l{i+1}'](h)
